...
const ArticleCard = ({ article, variant = "default" }) => {
  ...
  // ✅ Compat: mockData (image/category) + Sanity (coverImageUrl/section)
  const imageSrc = article.coverImageUrl || article.image || article.mainImageUrl || null;
  ...
  const readTime = article.readTime ? `${article.readTime}` : null;
